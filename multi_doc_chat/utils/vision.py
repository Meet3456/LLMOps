    ...
    for attempt in range(1,retries+1):
        try:
            ...
            response = await asyncio.wait_for(
                _caption_request(base_64,prompt,timeout,model,max_tokens,top_p,temperature),
                timeout = timeout+5
            )

            message = response.choices[0].message

            caption_text = message.content if isinstance(
...
async def caption_image(
    ...
    **kwargs
    ...
    try:
        ...
        return await caption_image_from_bytes(base64.b64decode(b64), prompt=prompt, **kwargs)
    except Exception as e:

...
from multi_doc_chat.utils.table import extract_tables_from_pdf, extract_tables_from_csv, html_tables_to_json
...
async def _process_single_path(p: Path , images_dir: Path, tables_dir: Path) -> List[Document]:
    """
            ...
            for i in range(len(pdf)):
                ...
                for img_index, img_meta in enumerate(image_list):
                    try:
                        ...
                        # Save image to disk
                        img_path = images_dir / f"{p.stem}_page{i+1}_img{img_index+1}.png"
...
async def load_documents_and_assets(paths: Iterable[Path] , images_dir: Path, tables_dir: Path) -> List[Document]:
